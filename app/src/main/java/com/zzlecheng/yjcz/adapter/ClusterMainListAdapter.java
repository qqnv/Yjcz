...
import com.zzlecheng.yjcz.utils.DensityUtil;
...
/**
 * @类名: ClusterMainListAdapter
 * @描述: 通讯录adapter
 * @作者: huangchao
 * @时间: 2019/1/18 10:12 AM
 * @版本: 1.0.0
 */
public class ClusterMainListAdapter extends BaseAdapter {
    ...
    public interface ButtonInterface {
        void onItemClick(View view, String id, String name, String az);
    }
    ...
    public ClusterMainListAdapter(final Context mcontext, List<TreePoint> pointList, HashMap<String, TreePoint> pointMap) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if (convertView == null) {
            convertView = LayoutInflater.from(mcontext).inflate(R.layout.item_tree, null);
            holder = new ViewHolder();
            holder.text = convertView.findViewById(R.id.text);
            holder.icon = convertView.findViewById(R.id.icon);
            holder.ib_select = convertView.findViewById(R.id.ib_select);
        ...
        if ("0".equals(tempPoint.getISLEAF())) {  //如果为父节点
            if (!tempPoint.isExpand()) {    //不展开显示加号
                ...
                holder.icon.setImageResource(R.mipmap.txl_dk);
            } else {                        //展开显示减号
                holder.icon.setVisibility(View.VISIBLE);
                holder.icon.setImageResource(R.mipmap.txl_sq);
            }
        } else {   //如果叶子节点，不占位显示
            holder.icon.setVisibility(View.INVISIBLE);
        }
        if (operateMode == ModeSelect) {
            if ("1".equals(tempPoint.getISLEAF())) {//若为1则是最后一层
            ...
            holder.ib_select.setOnClickListener(v -> {
                onModeSelect(tempPoint);
//                buttonInterface.onItemClick(v, tempPoint.getID(), tempPoint.getNNAME(), tempPoint.getAzkjid());
            });
        } else {
            holder.ib_select.setVisibility(View.GONE);
        }
    ...
    public void onItemClickAll(int position,View view) {
...
//            Toast.makeText(mcontext, getSubmitResult(treePoint), Toast.LENGTH_SHORT).show();
            buttonInterface.onItemClick(view,treePoint.getID(), treePoint.getNNAME(), treePoint.getAzkjid());
        } else {  //如果点击的是父类
    ...
    private void onModeSelect(TreePoint treePoint) {
        ...
        LogUtils.e(treePoint.isSelected() + "-" + treePoint.getID());
    }
